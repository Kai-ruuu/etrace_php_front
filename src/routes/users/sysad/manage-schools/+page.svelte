<script>
	...
	import SchoolAddModal from "$lib/components/grouped/users/admins/sysad/SchoolAddModal.svelte";
	import SchoolEditModal from "$lib/components/grouped/users/admins/sysad/SchoolEditModal.svelte";

	let archived = false;
	let addModalOpen = $state(false);
	let editModalOpen = $state(false);
	let editModalSchool = $state(null);
	...
	async function onAddCommit(school) {
		await SchoolService.create(
			...
			async (data, status) => {
				...
				await searchSchools();
				await Toast.fire({
					title: data?.message ?? "School has been created.",
					icon: "success"
				})
			},
			async (data, status) => {
				addModalOpen = false;
				await Toast.fire({
					title: data?.message ?? "Failed to create school.",
					icon: "error"
				})
			},
		)
	...
	async function onEdit(school) {
		editModalOpen = true;
		editModalSchool = school;
	}

	async function onEditCommit(school) {
		await SchoolService.rename(
			school,
			async (data, status) => {
				editModalOpen = false;
				await searchSchools();
				await Toast.fire({
					title: data?.message ?? "School has been edited.",
					icon: "success"
				});
			},
			async (data, status) => {
				editModalOpen = false;k
				await Toast.fire({
					title: data?.message ?? "Failed to rename school.",
					icon: "error"
				});
			},
		);
